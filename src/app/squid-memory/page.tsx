...
const getSymbolPool = (difficulty: Difficulty) => {
  if (difficulty === 'Easy') return symbols.slice(0, 6);
  if (difficulty === 'Medium') return symbols.slice(0, 12);
  return symbols;
};
...
export default function SquidMemoryGame({ activeGame }: { activeGame?: string }) {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-gradient-to-br from-zinc-900 via-black to-zinc-950">
      <section className="w-full max-w-6xl p-6 rounded-xl border border-pink-500 shadow-[0_0_30px_#ff00cc] relative">
        <h2 className="text-pink-400 text-2xl font-bold mb-4 tracking-wide text-center drop-shadow-[0_0_6px_#ff00cc]">
          Squid Memory Match
        </h2>
          ...
          {['Easy', 'Medium', 'Hard'].map(level => (
            <button
              ...
              className={`px-3 py-1 text-xs rounded border ${
                difficulty === level ? 'bg-pink-600 text-white' : 'bg-black text-zinc-400'
              }`}
            >
              {level}
            </button>
        ...
        <div className="flex justify-between items-center mb-6 text-sm text-zinc-400">
          ...
          <button
            onClick={handleReset}
            className="bg-pink-600 hover:bg-pink-700 text-white px-4 py-1 rounded-md border border-pink-400 text-xs shadow-md"
          >
            Reset Game
          </button>
        </div>
              ...
              {deck.map((symbol, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`relative h-20 w-20 rounded-lg border overflow-hidden transition-all duration-300 ${
                      isMatched
